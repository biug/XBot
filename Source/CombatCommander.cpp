...
void CombatCommander::initializeSquads()
{
	...
	bool mustSurvey = Config::Scout::MustScout.find(BWAPI::Broodwar->enemy()->getName()) != Config::Scout::MustScout.end() &&
		BWTA::getStartLocations().size() > 3;
	if ((BWAPI::Broodwar->self()->getRace() == BWAPI::Races::Zerg &&
		BWAPI::Broodwar->enemy()->getRace() != BWAPI::Races::Terran) || mustSurvey)
	{
...
void CombatCommander::updateAttackSquads()
{
    ...
    SquadOrder mainAttackOrder(SquadOrderTypes::Attack, getMainAttackLocation(&mainAttackSquad), (StateManager::Instance().being_rushed ? 200 : 800), "Attack enemy base");
...
void CombatCommander::updateBaseDefenseSquads() 
{
	...
	for (BWTA::Region * myRegion : InformationManager::Instance().getOccupiedRegions(BWAPI::Broodwar->self()))
	{
        ...
        else 
        {
            ...
            if (!_squadData.squadExists(squadName.str()))
            {
                SquadOrder defendRegion(SquadOrderTypes::Defend, regionCenter, 600, "Defend region");
...
BWAPI::Position CombatCommander::getMainAttackLocation(const Squad * squad)
{
	...
	// if being rushed, defense
	auto selfMain = InformationManager::Instance().getMainBaseLocation(BWAPI::Broodwar->self());
	if (StateManager::Instance().being_rushed && selfMain)
	{
		BWAPI::Unit bestSunken = nullptr;;
		for (const auto & unit : BWAPI::Broodwar->self()->getUnits())
		{
			if (unit->getType() == BWAPI::UnitTypes::Zerg_Sunken_Colony)
			{
				// 找离基地最远的
				if (!bestSunken || bestSunken->getDistance(selfMain->getPosition()) < unit->getDistance(selfMain->getPosition()))
				{
					bestSunken = unit;
				}
			}
		...
		{
			// guard point
			BWAPI::Broodwar->drawCircleMap(bestSunken->getPosition(), 20, BWAPI::Colors::Purple, true);
			return bestSunken->getPosition();
		}
	}
