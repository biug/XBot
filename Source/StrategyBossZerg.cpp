...
BuildOrder & StrategyBossZerg::freshProductionPlan()
{
	...
	if (_gasUnit != BWAPI::UnitTypes::None &&
		...
		!WorkerManager::Instance().isCollectingGas())
	{
		produce(MacroCommandType::StartGas);			//qi: 采集gas，这条命令挺好的
	}

	// Get zergling speed if at all sensible.			//qi: 小狗科技升级条件
	...
	// Ditto zergling attack rate.						//qi: 小狗科技升级条件
	...
	// Get hydra speed and range if they make sense.	//qi: 刺蛇科技升级条件
	...
	// Get lurker aspect if it's next.				//qi: 是否制造潜行者
	...
	// Move toward ultralisks.					// qi: 建造猛犸
	...
	// If we're in reasonable shape, get carapace upgrades.		//qi: 进化腔的地面部队装甲升级
	...
	// If we have 2 evos, or if carapace upgrades are done, also get melee attack.		// qi: 进化腔的攻击力升级
	// Coordinate upgrades with the nextInQueueIsUseless() check.
	...
	// If we have resources left, make units too.										//qi: 上面造完之后，还有剩余
