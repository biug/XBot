...
void MicroMelee::assignTargets(const BWAPI::Unitset & targets)
{
	...
	for (const auto target : targets) 
	{
		...
		{
			if (target->getType().isWorker() && StateManager::Instance().ignore_scout_worker)
			{
				...
				// if we should ignore worker, ingore scout worker
				if (enemyMain && enemyNatural)
				{
					if (target->getDistance(enemyMain->getPosition()) < 400 || target->getDistance(enemyNatural->getPosition()) < 400)
					{
