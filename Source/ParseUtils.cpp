...
void ParseUtils::ParseConfigStr()
{
	...
	// Parse the Scout Options
	if (doc.HasMember("Scout") && doc["Scout"].IsObject())
	{
		const rapidjson::Value & scout = doc["Scout"];
		if (scout.HasMember("Must") && scout["Must"].IsArray())
		{
			Config::Scout::MustScout.clear();
			const rapidjson::Value & bots = scout["Must"];
			for (size_t b(0); b < bots.Size(); ++b)
			{
				Config::Scout::MustScout.insert(bots[b].GetString());
			}
		}
		JSONTools::ReadInt("ScoutRound", scout, Config::Scout::ScoutRound);
	}

	if (doc.HasMember("AntiCannon") && doc["AntiCannon"].IsObject())
	{
		const rapidjson::Value & cannon = doc["AntiCannon"];
		if (cannon.HasMember("CannonProtoss") && cannon["CannonProtoss"].IsArray())
		{
			Config::AntiCannon::CannonProtoss.clear();
			const rapidjson::Value & bots = cannon["CannonProtoss"];
			for (size_t b(0); b < bots.Size(); ++b)
			{
				Config::AntiCannon::CannonProtoss.insert(bots[b].GetString());
			}
		}
	}
