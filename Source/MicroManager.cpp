...
void MicroManager::execute(const SquadOrder & inputOrder)
{
	...
	{
		auto ourMain = InformationManager::Instance().getMyMainBaseLocation();
		auto enemyMain = InformationManager::Instance().getEnemyMainBaseLocation();
		for (const auto unit : _units) 
		{
			if (!StateManager::Instance().being_rushed || (ourMain && unit->getDistance(ourMain->getPosition()) < 800))
			{
				MapGrid::Instance().GetUnits(nearbyEnemies, unit->getPosition(), unit->getType().sightRange(), false, true);
			}
			if (StateManager::Instance().being_rushed && enemyMain && unit->getDistance(enemyMain->getPosition()) < 800)
			{
				MapGrid::Instance().GetUnits(nearbyEnemies, order.getPosition(), order.getRadius(), false, true);
			}
		}
	...
	else if (order.getType() == SquadOrderTypes::Defend)
	{
		...
		for (const auto unit : _units)
		{
			if (!StateManager::Instance().being_rushed || (ourMain && unit->getDistance(ourMain->getPosition()) < 400))
			{
